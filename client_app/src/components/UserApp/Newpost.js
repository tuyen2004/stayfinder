...
function NewPost() {
  ...
  const nextImage = (postIndex) => {
    setCurrentIndexes((prevIndexes) => {
      ...
      const images = listpost[postIndex].images
        ? listpost[postIndex].images.split(",")
        : [];
  ...
  const prevImage = (postIndex) => {
    setCurrentIndexes((prevIndexes) => {
      ...
      const images = listpost[postIndex].images
        ? listpost[postIndex].images.split(",")
        : [];
      newIndexes[postIndex] =
        (newIndexes[postIndex] - 1 + images.length) % images.length; // Chuyển đến ảnh trước đó
      return newIndexes;
    });
  };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: "20px" }}>
        <p
          style={{ marginBottom: "15px", fontWeight: "bold", fontSize: "20px" }}
        >
          TIN MỚI
        </p>
            ...
            return (
              <div className="Product" key={index}>
                ...
                <div style={{ padding: "0px 10px" }}>
                  ...
                  >
                    <p style={{ color: "#2E73A0" }}>
                      {post.room_price} triệu/tháng
                    </p>
                    <p style={{ marginRight: "17px" }}>{post.room_size} m²</p>
                  </div>
                  <p style={{ overflowWrap: "break-word" }}>
                    {getDistrict(post.id_district)}
                  </p>
                  <p>
                    Ngày:{" "}
                    {moment(post.date_create).locale("vi").format("DD/MM/YYYY")}
                  </p>
